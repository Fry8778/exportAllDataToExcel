...
async function loadDataAndExportToExcel() {
    try {
        // Загрузка страницы
        const response = await fetch('https://novus.online/category/melena');
        ...
        }, 1000); // Задержка в 500 миллисекунд (1 секунда)
    } catch (error) {
...
function novusExportToExcel(container) {     
    const productCards = container.querySelectorAll('.base-is-link.base-card.catalog-products__item');
    const filteredProducts = Array.from(productCards).filter(productCard => {
        const productNameElement = productCard.querySelector('.base-card__label > span');           
...
export {loadDataAndExportToExcel};
export { novusExportToExcel };